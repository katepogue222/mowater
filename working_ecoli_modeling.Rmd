...
pilot_eps_removed <- read_csv("data/pilot_eps_filled.csv")
control_eps_filled <- read_csv("data/control_eps_filled.csv")
control_eps_removed <- read_csv("data/control_eps_removed.csv" )
```
...
legend("right",
       legend = c(min(cor_matrix), max(cor_matrix)),
       fill = colorRampPalette(c("blue", "white", "red"))(2),
       title = "Correlation",
       cex = 0.8,
       box.col = NA)
...
#fitting full model
mod_full <- lm(data = train[, ], log_pilot_ecoli~.)
...
l <- lapply(1:nrow(subset), function(i){
  ...
  #Creating tibble with log_pilot_ecoli as first column
  tb <- tibble("log_pilot_ecoli" = train$log_pilot_ecoli)

  #Adding each variable to the tibble
  for (j in 1:length(predictors[[1]])) {
  
  tb[,1+j] <- get(predictors[[1]][j])
  colnames(tb) <- c("log_pilot_ecoli", predictors[[1]])
  }

  mod <- lm(data = tb, log_pilot_ecoli ~.) #Fitting model with variables selected through the loop
  
  pred <- predict(mod, newdata = test) #Computing the model predictions
  rmse <- rmse(test$log_pilot_ecoli, pred) #Computing the model RMSE
  
  # Allocating Model Selection criteria to the list
  list("predictors" = subset$predictors[i] |> unlist(), 
       "rmse" = rmse(test$log_pilot_ecoli, pred), 
...
model <- lm(log_pilot_ecoli ~ `AB2A_DO_mg_per_L` + `NSI_TEMP_C` + `AB2_INF_FLOW_MGD` + `Pilot_Floc_SRT_d`, data = pilot_eps_filled)
summary(model)
plot(model)
```
...
#fitting full model
mod_full <- lm(data = train[, c(2:15)], log_pilot_ecoli~.)
